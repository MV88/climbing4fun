<template>
  <div>
    <b-modal id="attemptEditForm" title="Attempt Form" hide-footer>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$axios
        .$patch(`/api/v1/attempts/${this.form.id}`, this.form, {
          headers: {
        ...
        .then((data) => {
          this.$emit("updateListItem", this.form.id, data.result);
        })
        .catch((e) => {
          console.error(e);
        });

      this.$bvModal.hide("attemptEditForm");
    },
    onReset(event) {
